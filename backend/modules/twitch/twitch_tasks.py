...
def p_twitch(username, from_m, level):
    ...
    try:
        for qty_video, (video, _comments) in enumerate(
            ...
            # Thumbnail
            photos_item = {
                "name-node": "Twitch" + str(qty_video),
                "title": "Video " + str(qty_video),
                "picture": video.thumbnail_url.replace("%{width}", "300").replace(
                    "%{height}", "300"
                ),
                "subtitle": "",
                "link": link,
    ...
    qty_followers = 0
    try:
        for _f in helix.user(username).followers():
            qty_followers += 1
    except Exception:
        pass
    ...
    qty_followed = 0
    try:
        for _f in helix.user(username).following():
            qty_followed += 1
    except Exception:
        pass
    ...
    raw_node_total = []
    raw_node_total.append({"status": "Ok", "code": 0})
