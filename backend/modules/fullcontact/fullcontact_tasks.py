...
def p_fullcontact(email):
    ...
    if key and len(key) < 20:
        ...
        raw_node = {"status": 400, "message": "400: No access token"}
    ...
    if raw_node.get("status", "") != "" and (
        ...
        if raw_node.get("status", "") == 200:
            if raw_node.get("contactInfo", "") != "":
                profile_item = {
                    "name": raw_node.get("contactInfo", "").get("fullName", "")
                }
                profile.append(profile_item)
                profile_item = {
                    "firstName": raw_node.get("contactInfo", "").get("givenName", "")
                }
                profile.append(profile_item)
                profile_item = {
                    "lastName": raw_node.get("contactInfo", "").get("familyName", "")
                }
                profile.append(profile_item)
            ...
            for social in raw_node.get("socialProfiles", ""):
                if (social.get("username", "") != "") and (social.get("id", "") != ""):
                    subtitle = social.get("username", "") + " - " + social.get("id", "")
                ...
                if social.get("typeId", "") == "github":
                    tasks.append(
                        {"module": "github", "param": social.get("username", "")}
                    )
                if social.get("typeId", "") == "keybase":
                    tasks.append(
                        {"module": "keybase", "param": social.get("username", "")}
                    )
                if social.get("typeId", "") == "twitter":
                    tasks.append(
                        {"module": "twitter", "param": social.get("username", "")}
                    )
                if social.get("typeId", "") == "linkedin":
                    tasks.append(
                        {"module": "linkedin", "param": social.get("username", "")}
                    )
                if social.get("typeId", "") == "instagram":
                    ...
                    tasks.append({"module": "instagram", "param": inst_user})

            if raw_node.get("demographics", "") != "":
                if raw_node.get("demographics", "").get("locationDeduced", "") != "":
                    if (
                        raw_node.get("demographics", "")
                        .get("locationDeduced", "")
                        .get("deducedLocation", "")
                        != ""
                    ):
                        profile_item = {
                            "location": raw_node.get("demographics", "")
                            .get("locationDeduced", "")
                            .get("deducedLocation", "")
                        }
                        profile.append(profile_item)
                    ...
                    profile_item = {
                        "location": raw_node.get("demographics", "").get(
                            "locationGeneral", ""
                        )
                    }
                    profile.append(profile_item)
                ...
                if raw_node.get("demographics", "").get("gender", "") != "":
                    profile_item = {
                        "gender": raw_node.get("demographics", "").get("gender", "")
                    }
                    profile.append(profile_item)

            for photos in raw_node.get("photos", ""):
                photo_item = {
                    "name-node": photos.get("typeName", ""),
        ...
        if raw_node.get("details", "") != "":
            if raw_node.get("details", "").get("name", "") != "":
                profile_item = {
                    "name": raw_node.get("details", "").get("name", "").get("full", "")
                }
                profile.append(profile_item)
                profile_item = {
                    "firstName": raw_node.get("details", "")
                    .get("name", "")
                    .get("given", "")
                }
                profile.append(profile_item)
                profile_item = {
                    "lastName": raw_node.get("details", "")
                    .get("name", "")
                    .get("family", "")
                }
                profile.append(profile_item)
            ...
            for social in raw_node.get("details", "").get("profiles", ""):
                ...
                if rrss.get("service", "") == "twitter":
                    tasks.append({"module": "twitter", "param": username})
