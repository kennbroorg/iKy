...
def simple_analysis(source, type_s, username, data, output):
    ...
    match_twitter = re.match(
        r"^(https:\/\/www.google.com\/url\?q=|)(http:\/"
        + r"\/www\.|https:\/\/www\.|http:\/\/|https:\/\/)"
        + r"?[a-z0-9\.]*?(twitter+)\.[a-z]{2,5}(:[0-9]"
        + r"{1,5})?\/(\w+)",
        data[1],
    )
    if match_twitter:
    ...
    match_github = re.match(
        ...
        + r"(http:\/\/www\.|https:\/\/www\.|http:\/\/|http"
        + r"s:\/\/)?[a-z0-9\.]*?(github+)\.[a-z]{2,5}(:[0-9"
        + r"]{1,5})?\/(.*?)(%3|\/|&)(.*)",
        data[1],
    )
    if match_github:
    ...
    s_email = re.findall(
        r"^([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,5})$", data[0]
    )
    s_email = s_email + re.findall(
        r"^([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,5})$", data[2]
    )
