...
def p_mastodon(username, from_m):
    ...
    if match:
        try:
            ...
            raise Exception("iKy - Invalid input parameters") from None
        try:
...
def one_user(username, user_data, server=""):
    ...
    profile_item = {"photos": [{"picture": info["avatar"], "title": "Mastodon"}]}
    profile.append(profile_item)
    ...
    presence.append(
        {
            "name": "mastodon",
            "children": [
                {"name": "followers", "value": info["followers_count"]},
                {"name": "following", "value": info["following_count"]},
            ],
        }
    )
