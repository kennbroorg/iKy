...
def p_linkedin(user):
    ...
    for index, cookie_fn in enumerate(
        ...
        try:
            for cookie in cookie_fn(domain_name=""):
                if (
                    "linkedin" in cookie.domain
                    and (cookie.name == "li_at" or cookie.name == "JSESSIONID")
    ...
    if match:
        ...
        raise Exception(
            "iKy - The cookies in apikeys are wrong or expired\n"
    ...
    if found:
        ...
        user_id = Counter([ids]).most_common(1)  # This is for compatibility code
        ...
        url = (
            "https://www.linkedin.com/voyager/api/identity/profiles/"
            + user_id[0][0]
            + "/skillCategory?includeHiddenEndorsers=true"
        )
        ...
        for key in profile_keys:
            if key == "certificationView":
                for certificate in profile_view.get(key).get("elements", ""):
                    ...
                    certificationView.append(
                        {
                            "name": "Cetificate : " + certificate.get("name", ""),
                            "desc": certificate.get("authority", ""),
                            "picture": picture,
                        }
                    )
                    if (
                        certificate.get("timePeriod", "") != ""
                        and certificate.get("timePeriod", "").get("endDate", "") != ""
                    ):
                        timeline.append(
                            {
                                "action": "Start : " + certificate.get("name", ""),
                                "desc": certificate.get("authority", ""),
                    ...
                    if (
                        ...
                        timeline.append(
                            {
                                "action": "End : " + certificate.get("name", ""),
                                "desc": certificate.get("authority", ""),
                                "icon": "fas fa-certificate",
                                "date": date_convert(
                                    certificate.get("timePeriod", "").get("endDate", "")
                                ),
                            }
                        )
            ...
            if key == "educationView":
                ...
                for education in profile_view.get(key).get("elements", ""):
                    ...
                    certificationView.append(
                        {
                            "name": "Education : " + education.get("degreeName", ""),
                    ...
                    if (
                        education.get("timePeriod", "") != ""
                        and education.get("timePeriod", "").get("startDate", "") != ""
                    ):
                        timeline.append(
                            {
                                "action": "Start : " + education.get("degreeName", ""),
                                "desc": education.get("schoolName")
                                + " - "
                                + education.get("fieldOfStudy", "")
                                + " - "
                    ...
                    if (
                        education.get("timePeriod", "") != ""
                        and education.get("timePeriod", "").get("endDate", "") != ""
                    ):
                        timeline.append(
                            {
                                "action": "End : " + education.get("degreeName", ""),
                                "desc": education.get("schoolName")
                                + " - "
                                + education.get("fieldOfStudy", "")
                                + " "
            ...
            if key == "positionGroupView":
                ...
                for positionGroup in profile_view.get(key).get("elements", ""):
                    ...
                    #     "title", "")})
                    positionGroupView.append(
                        {
                            "label": positionGroup.get("name", "")
                            + " - "
                            + positionGroup.get("positions")[0].get("title", "")
                        }
                    )
                    if (
                        positionGroup.get("timePeriod", "") != ""
                        and positionGroup.get("timePeriod", "").get("startDate", "")
                        != ""
                    ):
                        timeline.append(
                            {
                                "action": "Start : " + positionGroup.get("name", ""),
                                "desc": positionGroup.get("positions")[0].get(
                                    "title", ""
                                ),
                                "icon": "fas fa-briefcase",
                                "date": date_convert(
                                    positionGroup.get("timePeriod", "").get(
                                        "startDate", ""
                                    )
                                ),
                            }
                    ...
                    if (
                        positionGroup.get("timePeriod", "") != ""
                        and positionGroup.get("timePeriod", "").get("endDate", "") != ""
                    ):
                        timeline.append(
                            {
                                "action": "End : " + positionGroup.get("name", ""),
                                "desc": positionGroup.get("positions")[0].get(
                                    "title", ""
                                ),
                                "icon": "fas fa-briefcase",
            ...
            if key == "profile":
                profile = []
                profile.append(
                    {"firstName": profile_view.get(key).get("firstName", "")}
                )
                profile.append({"lastName": profile_view.get(key).get("lastName", "")})
                profile.append(
                    {"organization": profile_view.get(key).get("headline", "")}
                )
                profile.append(
                    {"location": profile_view.get(key).get("locationName", "")}
                )

                if (
                    profile_view.get(key).get("miniProfile", "") != ""
                    and profile_view.get(key).get("miniProfile", "").get("picture", "")
                    != ""
                ):
                    rootUrl = (
                        profile_view.get(key)
                        .get("miniProfile", "")
                        .get("picture", "")
                        .get("com.linkedin.common.VectorImage", "")
                        .get("rootUrl", "")
                ...
                social_item = {
                    "name-node": "Name",
                    "title": "Name",
                    "subtitle": profile_view.get(key).get("firstName", "")
                    + " "
                ...
                social_item = {
                    "name-node": "Headline",
                    "title": "Headline",
                    "subtitle": profile_view.get(key).get("headline", ""),
                ...
                social_item = {
                    "name-node": "Location",
                    "title": "Location",
                    "subtitle": profile_view.get(key).get("locationName", ""),
