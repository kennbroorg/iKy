...
def p_emailrep(username):
    ...
    if "details" in raw_node:
        if "blacklisted" in raw_node["details"]:
            if not raw_node["details"]["blacklisted"]:
                icon_rep = "fas fa-thumbs-up"
            else:
                icon_rep = "fas fa-thumbs-down"
            gather_item = {
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Query EmailRep for an email address")
    parser.add_argument("email", help="Email address to look up")
    args = parser.parse_args()

    result = t_emailrep(args.email)
    output(result)
