...
def p_tinder(username, from_m="Initial"):
    ...
    file_path = Path.cwd() / "outputs" / "output-tinder.json"
    ...
    if soup.find(id="card-container"):
        raw_node = {
            "name": str(soup.find(id="name").text),
    ...
    if "status" not in raw_node:
        ...
        gather.append(gather_item)
        profile_item = {"photos": [{"picture": raw_node["picture"], "title": "Tinder"}]}
        profile.append(profile_item)
