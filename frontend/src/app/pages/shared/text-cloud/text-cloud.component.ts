    ...
    SVG3DTagCloud( element, params ) {
        ...
        function reInit() {
            ...
            if ( settings.width.toString().indexOf( '%' ) > 0 || settings.height.toString().indexOf( '%' ) > 0 ) {
                svgWidth = Math.round( width / 100 * parseInt( settings.width ) );
                svgHeight = Math.round( height / 100 * parseInt( settings.height ) );
            } else {
